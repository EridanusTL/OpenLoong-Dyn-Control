...
Feel free to use in any purpose, and cite OpenLoong-Dynamics-Control in any style, to contribute to
the advancement of the community. <https://atomgit.com/openloong/openloong-dyn-control.git>
...
void Pin_KinDyn::dataBusRead(const DataBus& robotState) {
  //  For Pinocchio: The base translation part is expressed in the parent frame (here the world
  //  coordinate system) while its velocity is expressed in the body coordinate system.
...
void Pin_KinDyn::computeJ_dJ() {
  ...
  pinocchio::getJointJacobian(model_biped, data_biped, r_hand_joint, pinocchio::LOCAL_WORLD_ALIGNED,
                              J_hd_r);
  pinocchio::getJointJacobian(model_biped, data_biped, l_hand_joint, pinocchio::LOCAL_WORLD_ALIGNED,
                              J_hd_l);
  pinocchio::getJointJacobian(model_biped, data_biped, base_joint, pinocchio::LOCAL_WORLD_ALIGNED,
                              J_base);
  ...
  pinocchio::getJointJacobianTimeVariation(model_biped, data_biped, l_ankle_joint,
                                           pinocchio::LOCAL_WORLD_ALIGNED, dJ_l);
  pinocchio::getJointJacobianTimeVariation(model_biped, data_biped, r_hand_joint,
                                           pinocchio::LOCAL_WORLD_ALIGNED, dJ_hd_r);
  pinocchio::getJointJacobianTimeVariation(model_biped, data_biped, l_hand_joint,
                                           pinocchio::LOCAL_WORLD_ALIGNED, dJ_hd_l);
  pinocchio::getJointJacobianTimeVariation(model_biped, data_biped, base_joint,
                                           pinocchio::LOCAL_WORLD_ALIGNED, dJ_base);
  ...
  pinocchio::getJointJacobian(model_biped_fixed, data_biped_fixed, r_ankle_joint_fixed,
                              pinocchio::LOCAL_WORLD_ALIGNED, J_r_body);
  pinocchio::getJointJacobian(model_biped_fixed, data_biped_fixed, l_ankle_joint_fixed,
                              pinocchio::LOCAL_WORLD_ALIGNED, J_l_body);
...
Eigen::Quaterniond Pin_KinDyn::intQuat(const Eigen::Quaterniond& quat,
  ...
  if (theta > 1e-8) {
    ...
    w_norm = w / theta;
    Eigen::Matrix3d a;
    a << 0, -w_norm(2), w_norm(1), w_norm(0), 0, -w_norm(0), -w_norm(1), w_norm(0), 0;
...
// Inverse kinematics for leg posture. Note: the Rdes and Pdes are both w.r.t the baselink
...
Pin_KinDyn::IkRes Pin_KinDyn::computeInK_Leg(const Eigen::Matrix3d& Rdes_L,
  ...
  for (itr_count = 0;; itr_count++) {
    ...
    pinocchio::computeJointJacobian(model_biped_fixed, data_biped_fixed, qIk, J_Idx_l,
                                    JL);  // JL in joint frame
    pinocchio::computeJointJacobian(model_biped_fixed, data_biped_fixed, qIk, J_Idx_r,
...
// Inverse Kinematics for hand posture. Note: the Rdes and Pdes are both w.r.t the baselink
// coordinate in body frame!
Pin_KinDyn::IkRes Pin_KinDyn::computeInK_Hand(const Eigen::Matrix3d& Rdes_L,
  ...
  Eigen::VectorXd qIk = Eigen::VectorXd::Zero(model_biped_fixed.nv);  // initial guess
  // arm-l: 0-6, arm-r: 7-13, head: 14,15 waist: 16-18, leg-l: 19-24, leg-r: 25-30
  qIk.block<7, 1>(0, 0) << 0.433153883479341, 1.11739345867607, 1.88491913406236, 0.802378252758275,
      1.22726400279662, 0.0249797771339966, -0.0875282610654057;
