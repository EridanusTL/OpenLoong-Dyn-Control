...
void JoyStickInterpreter::step() {
  ...
  vx_W = cos(thetaZ) * vx_L - sin(thetaZ) * vy_L;  // 计算世界坐标系下的X方向的速度
  vy_W = sin(thetaZ) * vx_L + cos(thetaZ) * vy_L;  // 计算世界坐标系下的Y方向的速度
  px_W += vx_W * dt;                               // 期望位置
  py_W += vy_W * dt;
}
...
void JoyStickInterpreter::setIniPos(const double posX, const double posY, const double posZ,
                                    const double thetaZ) {
  px_W = posX;
  py_W = posY;
  pz_W = posZ;
