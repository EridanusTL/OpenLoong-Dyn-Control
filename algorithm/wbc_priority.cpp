...
WBC_priority::WBC_priority(int model_nv_In, int QP_nvIn, int QP_ncIn, double miu_In, double dt)
  ...
  St_qpV2.block(6, 0, model_nv - 6, model_nv - 6) =
      Eigen::MatrixXd::Identity(model_nv - 6, model_nv - 6);
...
void WBC_priority::computeTau() {
  ...
  Eigen::MatrixXd eigen_qp_A1 =
      Eigen::MatrixXd::Zero(6, QP_nv);  // 18 means the sum of dims of delta_r and delta_Fr
  ...
  f_upp.block<8, 1>(0, 0) << 1e10, 1e10, 1e10, 1e10, f_z_upp, tau_upp_fe(0), tau_upp_fe(1),
      tau_upp_fe(2);
  f_upp.block<8, 1>(8, 0) = f_upp.block<8, 1>(0, 0);
  f_low.block<8, 1>(0, 0) << 0, 0, 0, 0, f_z_low, tau_low_fe(0), tau_low_fe(1), tau_low_fe(2);
...
void WBC_priority::computeDdq(Pin_KinDyn& pinKinDynIn) {
  ...
  /// -------- stand -------------
  {
    ...
    taskCtMap(2, 2) = 1;
    taskCtMap =
        fe_l_rot_cur_W * taskCtMap * fe_l_rot_cur_W.transpose();  // disable ankle roll joint
    ...
    kin_tasks_stand.taskLib[id].errX.block(0, 0, 2, 1) =
        pCoMDes.block(0, 0, 2, 1) - pCoMCur.block(0, 0, 2, 1);
    desRot = eul2Rot(base_rpy_des(0), base_rpy_des(1), base_rpy_des(2));
    ...
    kin_tasks_stand.taskLib[id].kd.block(2, 2, 3, 3) =
        Eigen::MatrixXd::Identity(3, 3) * 10;  // 100  // for hip rpy
...
void WBC_priority::copy_Eigen_to_real_t(qpOASES::real_t* target, const Eigen::MatrixXd& source,
                                        int nRows, int nCols) {
  int count = 0;
