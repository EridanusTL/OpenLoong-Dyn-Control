...
void FootPlacement::getSwingPos() {
  ...
  posDes_W = hipPos_W + KP * (desV_W - curV_W) * (-1) + 0.5 * tSwing * curV_W +
             curV_W * (1 - phi) * tSwing;  // 机器人平移时的落足点规划

  // for angular veloctity
  // 机器人旋转时的落足点规划，thetaF是目标落足点与机身连线在世界坐标系下的夹角
  double thetaF;
  thetaF = yawCur + theta0 + omegaZ_W * (1 - phi) * tSwing + 0.5 * omegaZ_W * tSwing +
           kp_wz * (omegaZ_W - desWz_W);
  ...
  double yOff_L =
      0.04;  // 0.01; // foot-end position offset in y direction in body frame, positive for moving the leg inside
  ...
  if (phi < 1.0) {
    pDesCur[0] = posStart_W(0) + (posDes_W(0) - posStart_W(0)) / (2 * 3.1415) *
                                     (2 * 3.1415 * phi - sin(2 * 3.1415 * phi));
    pDesCur[1] = posStart_W(1) + (posDes_W(1) - posStart_W(1)) / (2 * 3.1415) *
                                     (2 * 3.1415 * phi - sin(2 * 3.1415 * phi));
  }
