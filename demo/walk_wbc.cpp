...
int main(int argc, const char** argv) {
  ...
  WBC_priority WBC_solv(kinDynSolver.model_nv, 18, 22, 0.7,
                        mj_model->opt.timestep);             // WBC solver
  ...
  auto resLeg =
      kinDynSolver.computeInK_Leg(fe_l_rot_des, fe_l_pos_L_des, fe_r_rot_des, fe_r_pos_L_des);
  ...
  while (!glfwWindowShouldClose(uiController.window)) {
    ...
    while (mj_data->time - simstart < 1.0 / 60.0 &&
           uiController.runSim)  // press "1" to pause and resume, "2" to step the simulation
    {
      ...
      if (simTime > startWalkingTime) {
        jsInterp.setWzDesLPara(0, 1);  // 设置期望角速度(0)与到达期望角速度的时间(1s)
        jsInterp.setVxDesLPara(
            xv_des,
            2.0);  // jsInterp.setVxDesLPara(0.9,1); // 设置期望前进速度xv_des（朝向系）与到达期望速度的时间(2s)
        RobotState.motionState = DataBus::Walk;  // start walking
      } else
        jsInterp.setIniPos(RobotState.q(0), RobotState.q(1),
                           RobotState.base_rpy(
                               2));  // 若未达到前进时间，则期望速度与位置均设置为初始速度和初始位置

      if (simTime >= startSteppingTime) {
        jsInterp.step();
        jsInterp.setIniPos(RobotState.q(0), RobotState.q(1), stand_legLength + foot_height,
                           RobotState.base_rpy(2));
        jsInterp.dataBusWrite(
            RobotState);  // only pos x, pos y, pos_z, theta z, vel x, vel y , omega z are rewrote.
        // gait scheduler
        gaitScheduler.start();
      ...
      if (simTime > startWalkingTime + 1) {
        RobotState.des_delta_q.block<2, 1>(0, 0) << jsInterp.vx_W * mj_model->opt.timestep,
            jsInterp.vy_W * mj_model->opt.timestep;
      ...
      if (simTime <= startSteppingTime) {  // 站立阶段保持当前位置
      ...
      mj_interface.setMotorsTorque(RobotState.motors_tor_out);  // 为mujoco发送控制力矩
      ...
      printf("rpyVal=[%.5f, %.5f, %.5f]\n", RobotState.rpy[0], RobotState.rpy[1],
             RobotState.rpy[2]);
      printf("gps=[%.5f, %.5f, %.5f]\n", RobotState.basePos[0], RobotState.basePos[1],
             RobotState.basePos[2]);
